...
public class JsonSession {
	...
	public Session toSession(JsonNode root){
		...
		try {
			...
			List<Task> tasks = mapper.readValue(attribute, new TypeReference<List<Task>>() { });
			if(tasks != null)
				session.setTasks(tasks);
			else{
				Log.e(getClass().getName(), "Tasks JSON node was empty/null or doesn't exist");
			}
			attribute = root.get(RELATIONS);
			List<TaskRelation> relations = mapper.readValue(attribute, new TypeReference<List<TaskRelation>>() { });
			if(relations != null)
				session.setRelations(relations);
			else{
				Log.e(getClass().getName(), "TaskRelation JSON node was empty/null or doesn't exist");
			}
