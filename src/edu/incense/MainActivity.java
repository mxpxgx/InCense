...
public class MainActivity extends MainMenuActivity implements Runnable {
    ...
    /*
     Delegate class that do the sensing and collecting of data
     SensingActivity sensingActivity = null;
     */
		...
		startButton.setOnClickListener(new View.OnClickListener() {
			public void onClick(View view) {
				...
				if (userName == null || !(userName.length() > 0)) {
					...
					Toast.makeText(getBaseContext(),getResources().getText(R.string.alert_username),Toast.LENGTH_LONG).show();
	...
	public synchronized void run() {
		//if ( serviceIntent!=null ){//sensingActivity != null) {
			...
			try {
				...
				Log.e(getClass().getName(), "Failed to sleep for "+duration+" ms",e);
			}
		//}
		if(sessionTask != null){
			try {
	...
	private Handler handler = new Handler() {
		...
		public void handleMessage(Message msg) {
			suspendRecordingSession();
			//Intent intent = new Intent(MainActivity.this, ResultsListActivity.class); // ResultsActivity.class
	...
	// Start recording session and the thread from this class, show the progress dialog
	private synchronized void startSession(String username){
		
		//Show progress dialog
		progressDialog = ProgressDialog.show(this, "Collecting data", getResources().getText(R.string.inst_sensing));
		...
		//serviceIntent = new Intent(MainActivity.this, edu.incense.inutil.RecordingSession.class);
		//startService(serviceIntent);
	...
	// Suspend recording session and the thread from this class, dismiss the progress dialog
	private void suspendRecordingSession(){
		
		//Suspend recording session
		/*if(sensingActivity != null){
			sensingActivity.suspend();
			try {
				Thread.sleep(1000);
			} catch (Exception e) {
