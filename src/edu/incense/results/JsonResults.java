...
public class JsonResults {
	...
	public FileQueue toFileQueue(File file){
		...
		try {
			...
			attribute = root.get(FILEQUEUE);
			 Queue<ResultFile> queue = mapper.readValue(attribute, new TypeReference<Queue<ResultFile>>() { });
			if(queue != null)
				fileQueue.setFileQueue(queue);
			else{
				Log.i(getClass().getName(), "Queue JSON node was empty/null or doesn't exist");
			}
	...
	/*public void toJson(String fileName, FileQueue fileQueue){
		toJson(new File(fileName), fileQueue);
	}*/
	
	public void toJson(File file, FileQueue fileQueue){
		Log.i(getClass().getName(), "Writting file to: "+file.getAbsoluteFile());
		try {
