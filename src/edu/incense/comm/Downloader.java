...
public class Downloader extends Connection {
	...
	protected void setServersFromPreferences(){
		...
		serverProjectAddress = sp.getString("editTextProjectServerAddress","http://urbanmoments.dyndns.org:8182");
	}
	
	@Override
	protected void setPathsFromResource(){
		super.setPathsFromResource();
		serverProjectPath = context.getResources().getString(R.string.server_project_signature_path);
		serverProjectConfigPath = context.getResources().getString(R.string.server_project_path);
	}
	
	public boolean getProjectDataTo(String filePath){
		if(!connect(serverProjectAddress+serverProjectPath, ConnectionType.INPUT, JSON_TYPE))
			return false;
		return getData(filePath, false);
	}
	
	public boolean getProjectConfigTo(String filePath){
		if(!connect(serverProjectAddress+serverProjectConfigPath, ConnectionType.INPUT, JSON_TYPE))
			return false;
		return getData(filePath, false);
	}
