...
/***
 *  Uploader is  used to upload/send a file to an specific server depending on the type of file
 * @author mxpxgx
 *
 */
public class Uploader extends Connection {
	...
	protected void setServersFromPreferences(){
		...
		serverDbAddress = sp.getString("editTextDbServerAddress","http://urbanmoments.dyndns.org:8182");
	}
	
	@Override
	protected void setPathsFromResource(){
		super.setPathsFromResource();
		serverSinkPath = context.getResources().getString(R.string.server_sink_path);
		serverSurveyPath = context.getResources().getString(R.string.server_survey_path);
		serverMediaPath = context.getResources().getString(R.string.server_media_path);
	}
	
	public boolean postSinkData(String filePath){
		if(!connect(serverDbAddress+serverSinkPath, ConnectionType.OUTPUT, JSON_TYPE))
			return false;
	...
	private boolean postMediaData(String filePath, String contentType){
		if(!connect(serverDbAddress+serverMediaPath, ConnectionType.OUTPUT, contentType))
			return false;
		return postData(filePath, true);
	}
	
	public boolean postSurveyData(String filePath){
		if(!connect(serverDbAddress+serverSurveyPath, ConnectionType.OUTPUT, JSON_TYPE))
			return false;
		return postData(filePath, false);
	}
