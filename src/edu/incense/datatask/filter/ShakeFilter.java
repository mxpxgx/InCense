	...
	private Data seekForShake(Data data) {
		...
		if(!last){
			...
			if(speed > SHAKE_THRESHOLD){
				Log.i(getClass().getName(), "SHAKE detected with speed: "+speed);
				counter++;
				if(counter%1==0){
					Log.i(getClass().getName(), "DOUBLE SHAKE detected with speed: "+speed);
					return new BooleanData(true);
				}
				return new BooleanData(false);
			} else {
				Log.i(getClass().getName(), "SHAKE NOT detected with speed: "+speed);
				return new BooleanData(false);
			}
		}
